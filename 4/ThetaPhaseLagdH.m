...
% 
% User can uncomment vHPC portions below if vHPC mean power thresholding or 
%   overlap b/t dHPC and vHPC thresholds is desired.
%
%   CALLED BY:
%       Thesis4_AnalyzeAllVelDat.m
% 
% KJS init (as fxn): 2020-02-13
...
%     vthresh = cell(size(subjs)); %mean vHPC theta power per recording
%     vh_thetagood = cell(size(subjs)); % time epochs when vHPC theta is greater than its mean for that trial (not used, but used for 'overlap')
...
%     overlap = cell(length(subjs),4); % degree of overlap between vHPC and dHPC thresholds
...
for si = 1:length(subjs) %loop thru subjects
    ...
    clear il_th pl_th vh_th %remove vh_th  hereif this thresh is wanted later
    disp('.')
    
    %% Determine threshold based on mean dHPC theta power
    % "Data were taken from periods when hippocampal theta power was greater than its mean for that recording"
    t = 1:length(dh_ang_d); % time vector (2000 Hz sampling rate)
    t = repmat(t,[size(dh_ang_d,2) 1])'; % matrix to align with vh_pow [1200001 x nrex]
    dh_pow = dh_th.^2;
%     vh_pow = vh_th.^2; %theta power time series
        clear dh_th vh_th 
%     vthresh{si} = mean(vh_pow,1); %mean vHPC theta power per recording
    dthresh{si} = mean(dh_pow,1); %mean dHPC theta power per recording
%     vh_thetagood{si} = bsxfun(@gt,vh_pow,vthresh{si}); %logical array: values greater than thresh
    dh_thetagood{si} = bsxfun(@gt,dh_pow,dthresh{si}); %logical array: values greater than thresh
    clear dh_pow vh_pow 
        
    %% Determine degree of overlap between vHPC and dHPC thetagood indices
%     overlap{si,1} = bsxfun(@eq,vh_thetagood{si},dh_thetagood{si});
%     overlap{si,2} = sum(overlap{si,1}(:,:).*1,1); % number of timestamps that match
%     overlap{si,3} = overlap{si,2}(:,:)/2000; %seconds overlap per recording (2000 Hz sampling rate)
%     overlap{si,4} = overlap{si,2}(:,:)/size(il_ang_d,1); % percent overlap out of total time, per recording
...
%         dat_pct{si,2}(1,ri) = (size(find(vh_ang_d(:,ri).*vh_thetagood{si}(:,ri)),1) / size(t,1))*100; %using vHPC mean, ~33% of data included
    end
    clear t ri
...
%     il_ang_v = il_ang.*vh_thetagood{si};
%     pl_ang_v = pl_ang.*vh_thetagood{si};
%     dh_ang_v = dh_ang.*vh_thetagood{si};
%     vh_ang_v = vh_ang.*vh_thetagood{si};
    clear dh_thetagood

    isNZ = ~(dh_ang_d==0); %index of non-zero values (passed threshold)
%     isNZ = ~(vh_ang==0); %index of non-zero values
