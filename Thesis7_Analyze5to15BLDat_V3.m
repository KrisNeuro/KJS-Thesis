...
tstart=tic;
%mPFC-IL
[M_IL_theta_boot,F_IL_theta_boot,MF_IL_theta_p_test,MF_thetapjm_IL] = BootStat_MvF2(M_IL_theta,F_IL_theta);
[M_IL_gamma_boot,F_IL_gamma_boot,MF_IL_gamma_p_test,MF_gammapjm_IL] = BootStat_MvF2(M_IL_gamma,F_IL_gamma);
[M_IL_delta_boot,F_IL_delta_boot,MF_IL_delta_p_test,MF_deltapjm_IL] = BootStat_MvF2(M_IL_delta,F_IL_delta);
    clear F_IL_delta F_IL_gamma F_IL_theta M_IL_delta M_IL_gamma M_IL_theta

%dHPC
[M_DH_theta_boot,F_DH_theta_boot,MF_DH_theta_p_test,MF_thetapjm_DH] = BootStat_MvF2(M_DH_theta,F_DH_theta);
[M_DH_gamma_boot,F_DH_gamma_boot,MF_DH_gamma_p_test,MF_gammapjm_DH] = BootStat_MvF2(M_DH_gamma,F_DH_gamma);
[M_DH_delta_boot,F_DH_delta_boot,MF_DH_delta_p_test,MF_deltapjm_DH] = BootStat_MvF2(M_DH_delta,F_DH_delta);
    clear F_DH_delta F_DH_gamma F_DH_theta M_DH_delta M_DH_gamma M_DH_theta

%vHPC
[M_VH_theta_boot,F_VH_theta_boot,MF_VH_theta_p_test,MF_thetapjm_VH] = BootStat_MvF2(M_VH_theta,F_VH_theta);
[M_VH_gamma_boot,F_VH_gamma_boot,MF_VH_gamma_p_test,MF_gammapjm_VH] = BootStat_MvF2(M_VH_gamma,F_VH_gamma);
[M_VH_delta_boot,F_VH_delta_boot,MF_VH_delta_p_test,MF_deltapjm_VH] = BootStat_MvF2(M_VH_delta,F_VH_delta);
    clear F_VH_delta F_VH_gamma F_VH_theta M_VH_delta M_VH_gamma M_VH_theta

%mPFC-PL
[M_PL_theta_boot,F_PL_theta_boot,MF_PL_theta_p_test,MF_thetapjm_PL] = BootStat_MvF2(M_PL_theta,F_PL_theta);
[M_PL_gamma_boot,F_PL_gamma_boot,MF_PL_gamma_p_test,MF_gammapjm_PL] = BootStat_MvF2(M_PL_gamma,F_PL_gamma);
[M_PL_delta_boot,F_PL_delta_boot,MF_PL_delta_p_test,MF_deltapjm_PL] = BootStat_MvF2(M_PL_delta,F_PL_delta);
...
%% 1.2.1 Plot theta band joint-probability matrices: Male vs Female
[h121] = Plot_4pjm_MF(MF_thetapjm_IL,MF_thetapjm_DH,MF_thetapjm_VH,MF_thetapjm_PL,1,x);
    % Create textbox for figure title
    annotation(h121,'textbox',[0.308740324594257 0.944323314529585 0.427370786516854 0.0508054522924403],...
...
[h121b] = figure('units','normalized','outerposition',[0 0 1 1]);
a1 = subplot(221);
    [a1] = PlotHistBoot_MF(a1,M_IL_theta_boot,F_IL_theta_boot);
    title('mPFC-IL')
    legend Male Female
a2 = subplot(222);
    [a2] = PlotHistBoot_MF(a2,M_DH_theta_boot,F_DH_theta_boot);
    title('dHPC')
a3 = subplot(223);
    [a3] = PlotHistBoot_MF(a3,M_VH_theta_boot,F_VH_theta_boot);
    title('vHPC')
a4 = subplot(224);
    [a4] = PlotHistBoot_MF(a4,M_PL_theta_boot,F_PL_theta_boot);
    title('mPFC-PL')

% Save figures
% fd = [figdrOut 'Bandpower' filesep]; %figure output directory (set @section head)
fn = ['Theta-BandPowBootjpm_MvF']; %figure title
    saveas(h121,[fd fn '.png'])
    saveas(h121,[fd fn '.fig'])
    close(h121); clear h121
fn = ['Theta-BandPowBootHist_MvF'];
    saveas(h121b,[fd fn '.png'])
    saveas(h121b,[fd fn '.fig'])
    close(h121b); clear h121b a* fn
disp('Theta band power figures saved! (MvF)')


%% 1.2.2 Plot gamma band joint-probability matrices: Male vs Female
[h122] = Plot_4pjm_MF(MF_gammapjm_IL,MF_gammapjm_DH,MF_gammapjm_VH,MF_gammapjm_PL,1,x);
    % Create textbox for figure title
    annotation(h122,'textbox',[0.308740324594257 0.944323314529585 0.427370786516854 0.0508054522924403],...
    'String','Gamma band power: Joint-Probability matrices',...
    'HorizontalAlignment','center','FontWeight','bold','FontSize',21,...
    'FitBoxToText','off','EdgeColor',[0.941176470588235 0.941176470588235 0.941176470588235]);

[h122b] = figure('units','normalized','outerposition',[0 0 1 1]);
a1 = subplot(221);
    [a1] = PlotHistBoot_MF(a1,M_IL_theta_boot,F_IL_theta_boot);
    title('mPFC-IL')
%     legend Male Female
...
% Save figures
fn = ['Gamma-BandPowBootjpm_MvF']; %figure title
    saveas(h122,[fd fn '.png'])
    saveas(h122,[fd fn '.fig'])
    close(h122); clear h122
fn = ['Gamma-BandPowBootHist_MvF'];
    saveas(h122b,[fd fn '.png'])
    saveas(h122b,[fd fn '.fig'])
    close(h122b); clear h122b a* fn
disp('Gamma band power figures saved! (MvF)')

%% 1.2.3 Plot delta band joint-probability matrices: Male vs Female
[h123] = Plot_4pjm_MF(MF_gammapjm_IL,MF_gammapjm_DH,MF_gammapjm_VH,MF_gammapjm_PL,1,x);
    % Create textbox for figure title
    annotation(h122,'textbox',[0.308740324594257 0.944323314529585 0.427370786516854 0.0508054522924403],...
...
[h123b] = figure('units','normalized','outerposition',[0 0 1 1]);
a1 = subplot(221);
    [a1] = PlotHistBoot_MF(a1,M_IL_theta_boot,F_IL_theta_boot);
    title('mPFC-IL')
%     legend Male Female
...
% Save figures
fn = ['Delta-BandPowBootjpm_MvF']; %figure title
    saveas(h123,[fd fn '.png'])
    saveas(h123,[fd fn '.fig'])
    close(h123); clear h123
fn = ['Delta-BandPowBootHist_MvF'];
    saveas(h123b,[fd fn '.png'])
    saveas(h123b,[fd fn '.fig'])
    close(h123b); clear h1232b a* fn
